...
@Component({
  selector: 'app-travail-temps-jour',
  standalone: true,
  imports: [CommonModule, FormsModule, MatIconModule, MatButtonModule, MatAutocompleteModule, SaisieEditFormComponent],
  template: `
<div class="page">
  ...
  <div class="pg-header">
    <div class="pg-header__left">
      <div class="pg-icon pg-icon--green"><mat-icon>today</mat-icon></div>
      <div>
        <h1 class="pg-title">Temps passés par jour</h1>
        <p class="pg-sub">{{ filteredRows().length }} saisie(s) sur la période</p>
      </div>
    </div>
  ...
  <div class="table-toolbar">
    ...
    <select class="pg-size-select" [(ngModel)]="pageSize" (ngModelChange)="page.set(0)">
      @for (sz of pageSizes; track sz) {
        <option [ngValue]="sz">{{ sz }} / page</option>
      }
    </select>
    ...
    @if (loading()) {
      ...
      @if (totalPages() > 1) {
        <div class="pagination">
          ...
          <span class="pag-info">Lignes {{ page() * pageSize + 1 }} à {{ Math.min((page() + 1) * pageSize, filteredRows().length) }} — Total : {{ filteredRows().length }} résultats</span>
  ...
  styles: [`
    ...
    .pg-icon--green { background:linear-gradient(135deg,#059669,#047857); box-shadow:0 4px 14px rgba(5,150,105,.35); }
    ...
    /* Filter bar */
    .filter-bar { display:flex; align-items:flex-end; gap:12px; padding:18px 28px 16px; flex-wrap:wrap; flex-shrink:0; background:#fff; margin:16px 28px 0; border-radius:12px; box-shadow:0 1px 6px rgba(0,0,0,.06); border:1px solid #f1f5f9; }
    ...
    .tot-green .tot-val { color:#15803d; }
    .tot-green .tot-lbl { color:#15803d; }
    .tot-red { background:#fff1f2; border-color:#fecdd3; }
    .tot-red .tot-val { color:#dc2626; }
    .tot-red .tot-lbl { color:#dc2626; }
    .tot-blue { background:#eff6ff; border-color:#bfdbfe; }
    .tot-blue .tot-val { color:#1d4ed8; }
    .tot-blue .tot-lbl { color:#1d4ed8; }
    ...
    .pg-size-select { height:32px; border:1px solid #e2e8f0; border-radius:6px; padding:0 8px; font-size:12px; background:#fff; color:#374151; }
...
export class TravailTempsJourComponent implements OnInit, OnDestroy {
  ...
  // NB : méthodes normales (et non computed()) car elles dépendent aussi de champs
  // classiques liés par ngModel (recherche, pageSize) — un computed() Angular ne
  // re-déclenche que sur lecture de signal, pas sur ces propriétés simples. Comme le
  // reste du module (totalFacturable(), sumFacturable()…), on relit à chaque cycle.
  filteredRows(): any[] {
  ...
  pageRows(): any[]   { return this.filteredRows().slice(this.page() * this.pageSize, (this.page() + 1) * this.pageSize); }
  totalPages(): number { return Math.max(1, Math.ceil(this.filteredRows().length / this.pageSize)); }
  ...
  vider() {
    this.dateDebut = this.dateFin = this.collaborateurId = this.clientId = this.recherche = '';
