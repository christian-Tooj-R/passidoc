...
<div class="page">
  ...
  <div class="pg-header">
    <div class="pg-header__left">
      <div class="pg-icon pg-icon--amber"><mat-icon>calendar_month</mat-icon></div>
      <div>
        <h1 class="pg-title">Temps passés par mois</h1>
              ...
              <td>
                @if (r.total > 0) {
                  <div class="mini-histo">
                    <div class="mini-bar-fact" [style.width.%]="r.facturable / maxTotal() * 100" [title]="'Facturable : ' + formatH(r.facturable)"></div>
                    <div class="mini-bar-nf"   [style.width.%]="r.nonFacturable / maxTotal() * 100" [title]="'NF : ' + formatH(r.nonFacturable)"></div>
                  </div>
                }
              </td>
            </tr>
  ...
  styles: [`
    ...
    .pg-icon--amber { background:linear-gradient(135deg,#d97706,#b45309); box-shadow:0 4px 14px rgba(217,119,6,.35); }
    ...
    .mois-badge { font-size:13px; font-weight:700; color:#1e293b; }

    .mini-histo { display:flex; flex-direction:column; gap:2px; min-width:80px; }
    .mini-bar-fact, .mini-bar-nf { height:7px; border-radius:3px; min-width:3px; transition:width .4s; }
    .mini-bar-fact { background:#22c55e; }
    .mini-bar-nf   { background:#f87171; }
...
export class TravailTempsMoisComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    const debut = new Date(now); debut.setMonth(debut.getMonth() - 12);
  ...
  maxTotal()      { return Math.max(...this.filteredRows().map(r => r.total), 1); }
