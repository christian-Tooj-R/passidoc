...
export class TasksScheduler {
  ...
  async notifierEcheancesProches(): Promise<number> {
    try {
      ...
      this.logger.error('[J-1] Erreur cron notifierEcheancesProches', err);
      return 0;
    }
  }
  ...
  async reporterTachesEnRetard(): Promise<number> {
    try {
      ...
      this.logger.error('[Report] Erreur cron reporterTachesEnRetard', err);
      return 0;
    }
  }
  ...
  async creerTachesRecurrentes(): Promise<number> {
    try {
      ...
      this.logger.error('[Récurrence] Erreur cron creerTachesRecurrentes', err);
      return 0;
    }
  }
}
