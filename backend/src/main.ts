...
import { runSalaireBaseMigrationIfNeeded } from './bootstrap-salaire-base-migration';

async function bootstrap() {
  await runSalaireBaseMigrationIfNeeded();
  ...
  app.use(helmet({
    contentSecurityPolicy: {
      directives: {
        ...
        imgSrc: ["'self'", 'data:', 'http://localhost:3000', 'blob:'],
      },
    },
  ...
  const allowedOrigins = (process.env.FRONTEND_URL || 'http://localhost:4200,http://localhost:4202').split(',');
  app.enableCors({
