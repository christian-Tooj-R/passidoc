...
/**
 * Paramétrage des rubriques de paie (cotisations, primes fixes, retenues fixes...).
 * Mutations réservées à l'admin/expert-comptable : ce sont les taux appliqués à tous les
 * bulletins du régime concerné.
 */
@ApiTags('Paie — Rubriques (paramétrage)')
@ApiBearerAuth()
@UseGuards(JwtAuthGuard, RolesGuard)
@Controller('paie/rubriques')
export class RubriquesPaieController {
  constructor(private service: RubriquesPaieService) {}

  @Post()
  @Roles(UserRole.ADMIN, UserRole.EXPERT_COMPTABLE)
  @ApiOperation({ summary: 'Créer une rubrique de paie' })
  create(@Body() dto: CreateRubriquePaieDto, @Req() req: any) {
  ...
  update(@Param('id', ParseIntPipe) id: number, @Body() dto: UpdateRubriquePaieDto, @Req() req: any) {
    return this.service.update(id, dto, req.user.tenantId);
  }

  @Delete(':id')
  @Roles(UserRole.ADMIN, UserRole.EXPERT_COMPTABLE)
  @ApiOperation({ summary: 'Supprimer une rubrique de paie' })
